...
> Employee ApiEmployeeGet(ctx).Execute()
...
func main() {
    ...
    resp, r, err := apiClient.EmployeeAPI.ApiEmployeeGet(context.Background()).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `EmployeeAPI.ApiEmployeeGet``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `ApiEmployeeGet`: Employee
    fmt.Fprintf(os.Stdout, "Response from `EmployeeAPI.ApiEmployeeGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiApiEmployeeGetRequest struct via the builder pattern
...
> Employee ApiEmployeeIdPut(ctx, id).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.EmployeeAPI.ApiEmployeeIdPut(context.Background(), id).Execute()
    if err != nil {
